...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withExceptions(function (Exceptions $exceptions) {
        ...
        // Validation errors
        $exceptions->render(
            fn (\Illuminate\Validation\ValidationException $e, $req) => ApiResponse::error('Validation error.', $e ? $e->errors() : [], Response::HTTP_UNPROCESSABLE_ENTITY)
        );

        // Authorization errors (Spatie)
        $exceptions->render(
            fn (\Spatie\Permission\Exceptions\UnauthorizedException $e, $req) => ApiResponse::error('You do not have the required authorization.', [], Response::HTTP_FORBIDDEN)
        );

        $exceptions->render(
            fn (OtpVerificationException $e, $req) => ApiResponse::error($e->getMessage(), [], Response::HTTP_BAD_REQUEST)
        );
